...
export class PersonalInformation implements OnInit, OnDestroy {
  ...
  loadUserData() {
    ...
    if (userStr) {
      try {
        ...
        // Load thêm thông tin chi tiết từ userInfo nếu có
        ...
        if (savedDetails) {
          ...
          this.userInfo.avatar = details.avatar || 'assets/image/avt.png';
        }
      ...
      if (savedData) {
        ...
        this.userInfo = { ...this.userInfo, ...JSON.parse(savedData) };
  ...
  // Save user data
  saveUserData() {
    // Lưu thông tin chi tiết vào userInfo
  ...
  onSubmitPasswordChange() {
    ...
    localStorage.setItem('userPasswordLength', this.passwordData.newPassword.length.toString());
    console.log(
      ' [PersonalInfo] Saved passwordLength to localStorage:',
      this.passwordData.newPassword.length
    );
    ...
    // alert('Mật khẩu đã được thay đổi thành công!'); // Comment out - user will see updated password display instead
