...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule, RouterModule, ReviewFormComponent],
  ...
  loadReviewedItemsFromBackend(): void {
    ...
    try {
      ...
      Promise.all(reviewPromises).then(() => {
        ...
        localStorage.setItem('reviewedItems', JSON.stringify(this.allReviewedItems));
        console.log(
          ' Loaded reviewed items from backend with full data (filtered by CustomerID)'
        );
      });
    } catch (error) {
      ...
      if (mappedOrder.products && mappedOrder.products.length > 0) {
        ...
        if (!isOrderReviewed) {
          this.unreviewedItems.push({
            ...
            orderDate: this.formatDate(mappedOrder.orderDate || mappedOrder.deliveryDate),
