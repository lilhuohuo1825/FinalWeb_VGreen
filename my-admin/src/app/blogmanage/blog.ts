...
export class Blog implements OnInit, AfterViewInit {
  ...
  viewBlogDetail(blog: BlogPost): void {
    ...
    // Ensure email and hashtags are set
    if (!this.currentBlog.email) {
      this.currentBlog.email = '';
    }
    if (!this.currentBlog.hashtags) {
      this.currentBlog.hashtags = '';
    }
  ...
  saveCurrentBlog(): void {
    ...
    if (!this.currentBlog.content || this.currentBlog.content.trim() === '') {
    ...
    const blogData: any = {
      id: this.currentBlog._id || this.currentBlog.blog_id,
      ...
      hashtags: this.currentBlog.hashtags || '',
      img: '', // Có thể cần thêm logic để lấy ảnh từ content
      excerpt: this.currentBlog.content.substring(0, 200) || '', // Tạo excerpt từ content
    ...
    if (this.isNewBlog) {
      ...
      this.apiService.createBlog(blogData).subscribe({
        next: (response) => {
          ...
          this.notificationService.showSuccess('Đã thêm bài viết mới!');
        ...
        error: (error) => {
          ...
          this.notificationService.showError('Lỗi khi tạo bài viết: ' + (error.error?.message || error.message));
        }
      });
    } else {
      // Cập nhật blog hiện có qua API
      const blogId = this.currentBlog._id || this.currentBlog.blog_id;
      this.apiService.updateBlog(blogId, blogData).subscribe({
        next: (response) => {
          console.log('✅ Blog updated successfully:', response);
          this.notificationService.showSuccess('Đã lưu thay đổi bài viết!');
          
          // Reload blogs để lấy ngày cập nhật mới nhất
          this.loadBlogs();
    ...
    this.backToList();
        ...
        error: (error) => {
          ...
          this.notificationService.showError('Lỗi khi cập nhật bài viết: ' + (error.error?.message || error.message));
        }
      });
    }
  }
  ...
  openEditPopup(): void {
    ...
    // Trigger change detection to ensure hashtags are ready
    this.cdr.detectChanges();
  }
  ...
  closeEditPopup(): void {
    this.showEditPopup = false;
    // Trigger change detection to update hashtags after closing popup
    this.cdr.detectChanges();
  }
  ...
  saveFromPopup(): void {
    ...
    if (this.isNewBlog) {
      ...
      this.isNewBlog = false;
      // Trigger change detection to update hashtags
      this.cdr.detectChanges();
        },
        error: (error) => {
          console.error('❌ Error creating blog:', error);
      ...
      this.apiService.updateBlog(blogId, blogData).subscribe({
        next: (response) => {
          console.log('✅ Blog updated successfully:', response);
          this.notificationService.showSuccess('Đã lưu thay đổi!');
  ...
  private buildCategoryMap(products: any[]): void {
    ...
    // If there's a current blog, regenerate hashtags
    if (this.currentBlog) {
      console.log('🔄 Regenerating hashtags for current blog after categories loaded...');
      this.cdr.detectChanges();
    }
  }
   ...
   * Logic: 
   * 1. Get blog's category (e.g., "Trái cây")
   * 2. Find subcategories/keywords from products in that category
   * 3. Search blog content for these keywords
   * 4. Create hashtags from found keywords
   */
  generateAutoHashtags(blog: BlogPost): string[] {
    ...
    // Get blog content (title + content)
    const blogText = `${blog.title || ''} ${blog.content || ''}`.toLowerCase();
    
    // Remove HTML tags and normalize whitespace
    const cleanText = blogText.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    console.log(`📋 Available categories in map:`, Array.from(this.categoryToSubcategories.keys()));
    
    // Get blog's category (lowercase for matching)
    const blogCategory = blog.category ? blog.category.toLowerCase().trim() : '';
    
    if (!blogCategory) {
      console.warn('⚠️ Blog has no category, cannot generate hashtags');
      return [];
    }
    
    console.log(`🔍 Looking for category: "${blogCategory}"`);
    
    // Find keywords from products in the blog's category
    ...
    if (this.categoryToSubcategories.has(blogCategory)) {
      ...
      console.log(`✅ Found exact category match: ${blogCategory}`);
    } else {
      // Try partial match (e.g., "Trái cây" matches "trái cây")
      ...
      for (const [category, keywords] of this.categoryToSubcategories.entries()) {
        ...
        if (blogCategory === normalizedCategory || 
            blogCategory.includes(normalizedCategory) || 
            normalizedCategory.includes(blogCategory)) {
          relevantKeywords = keywords;
          console.log(`✅ Found partial category match: "${category}" for "${blogCategory}"`);
          break;
        }
      }
    }
    
    // If no match found for specific category, search ALL keywords from ALL categories
    // This ensures we can still find hashtags even if category doesn't match exactly
    if (!relevantKeywords || relevantKeywords.size === 0) {
      console.warn(`⚠️ No keywords found for category: "${blogCategory}"`);
      console.log(`💡 Available categories:`, Array.from(this.categoryToSubcategories.keys()));
      console.log(`🔄 Falling back to search ALL keywords from ALL categories...`);
      ...
      console.log(`📝 Now searching ${relevantKeywords.size} keywords from all categories...`);
    } else {
      console.log(`🔍 Checking ${relevantKeywords.size} keywords from category "${blogCategory}"...`);
      console.log(`📝 Sample keywords:`, Array.from(relevantKeywords).slice(0, 10).join(', '));
    }
    
    // Sort keywords by length (longer first) to match "dâu tây" before "dâu"
    const sortedKeywords = Array.from(relevantKeywords).sort((a, b) => b.length - a.length);
    
    // Track matched text to avoid duplicates
    ...
    sortedKeywords.forEach(keyword => {
      ...
      // Skip empty keywords
      if (!keywordLower || keywordLower.length < 2) return;
      
      // Skip if already matched (for shorter versions)
      if (matchedTexts.has(keywordLower)) return;
      
      // Find all occurrences of keyword in content
      // Use simple includes first for Vietnamese (more flexible)
      let found = false;
      
      // Method 1: Simple includes (most flexible for Vietnamese)
      if (cleanText.includes(keywordLower)) {
        found = true;
      }
      
      // Method 2: Try with word boundaries (for better accuracy)
      if (!found) {
      ...
      // Method 3: Try with non-word characters (for Vietnamese)
      if (!found) {
        const escapedKeyword = keywordLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const simpleRegex = new RegExp(`(^|[^\\w])${escapedKeyword}([^\\w]|$)`, 'gi');
        const matches = cleanText.match(simpleRegex);
        if (matches && matches.length > 0) {
          found = true;
        }
      }
      ...
      if (found) {
        ...
        matchedTexts.add(keywordLower);
        console.log(`  ✓ Found keyword: "${keyword}" → hashtag: "${formattedHashtag}"`);
      }
    });
    
    // Convert to array and sort by appearance order in content
    const hashtagArray = Array.from(hashtags);
    
    // Sort by first occurrence in content
    hashtagArray.sort((a, b) => {
      const aLower = a.toLowerCase();
      const bLower = b.toLowerCase();
      const aIndex = cleanText.indexOf(aLower);
      const bIndex = cleanText.indexOf(bLower);
    ...
    console.log(`✅ Generated ${hashtagArray.length} hashtags:`, hashtagArray.join(', '));
  ...
  getAutoGeneratedHashtagsArray(): string[] {
    ...
    if (this.categoryToSubcategories.size === 0) {
      console.warn('⚠️ Categories not loaded, attempting to load...');
      this.loadProductCategories();
      // Return empty for now, will update after categories are loaded
      return [];
    }
    ...
    // Log for debugging
    if (hashtags.length === 0 && this.currentBlog.category) {
      console.log(`⚠️ No hashtags generated for blog "${this.currentBlog.title}" with category "${this.currentBlog.category}"`);
      console.log(`📋 Available categories:`, Array.from(this.categoryToSubcategories.keys()));
    }
  ...
  updateBlogContent(event: Event): void {
    ...
    // Trigger change detection to update hashtags display
